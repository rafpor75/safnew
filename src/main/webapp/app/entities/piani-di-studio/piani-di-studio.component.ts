...
export class PianiDiStudioComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.pianiDiStudioService
            ...
            .subscribe(
                (res: HttpResponse<IPianiDiStudio[]>) => this.paginatePianiDiStudios(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/piani-di-studio'], {
            queryParams: {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: IPianiDiStudio) {
        return item.id;
    }
    ...
    private paginatePianiDiStudios(data: IPianiDiStudio[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        this.queryCount = this.totalItems;
        this.pianiDiStudios = data;
    }
